...
namespace UWPRef.Controls
{
    public sealed partial class PageHeader : UserControl
    {
        ...
        private void OnControlsSearchBoxTextChanged(AutoSuggestBox sender, AutoSuggestBoxTextChangedEventArgs args)
        {
            ...
            {
                var suggestions = new List<NavigationInfo>();

                suggestions.AddRange(NavigationInfoDataSource.Instance.Items.Where(i => i.Title.IndexOf(sender.Text, StringComparison.CurrentCultureIgnoreCase) >= 0));
        ...
        {
            if (args.ChosenSuggestion != null && args.ChosenSuggestion is NavigationInfo)
            {
                var itemId = (args.ChosenSuggestion as NavigationInfo).UniqueId;
